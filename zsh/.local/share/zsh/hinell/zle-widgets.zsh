...
__git_diff_edit(){
	local 0="zsh: widgets: git diff"
	echo "${0}: ${EDITOR} \$(git )"
	local _diff_revisions_behind=$(git rev-list --count @)
	...
	# 	"1" \
	# 	3>&1 1>&2 2>&3) 
	echo -n "pecify number of revisions relative to the HEAD to get files of:"
	zle -I # allow input into this widget
	read _diff_revisions_depth\?'git diff @~' < /dev/tty
	[[ ${_diff_revisions_depth} =~ [[:digit:]] && ${_diff_revisions_depth} -gt 1 ]] || _diff_revisions_depth=1

	# files to open
	local _files=()
	if (( _diff_revisions_behind > 1 )); then
		_files=($(git diff --name-only @~${_diff_revisions_depth} ./))
	fi
	...
	if (( ${#_files[@]} == 0 )); then
		...
		then
			...
			echo -e "$(tput setaf 3)❌$(tput op).  Aborting."
			return
		fi
	fi

	if (( ${#_files[@]} == 0 )); then
		echo -e "${0}:$(tput setaf 5)info$(tput op): git diff --name-only produced no files. Using ls-tree for current HEAD..." 
		_files=($(git ls-tree --name-only @))
	fi
	# set up GIT_REPO_DIR
	__git_find_gitdir 
...
__git_config_edit(){
	...
	then
		...
		_editor_run "git config --edit" $HOME/.gitconfig 
